...
export function renderChart(htmlElement:any, {width, height, demo=true}:any) {
    chart = createChart(htmlElement.current, {
        width:  width,
      height:   height,
      layout: {
    ...
    if(demo){
        demoChart(data, candleSeries);
    }

    return chart;
}

export function updateChart(options:any) {
    chart.applyOptions({
      width: options.width,
...
function demoChart(data:any, candleSeries:any){
